...
| **S76** | **Kule/asker dalı adları çevrilecek mi?** (`data/towers.ts` T3 `branchName`: Keskin Nişancı, Kundakçı, Havan, Barut Fıçısı, Yıldırım, Buz; `data/barracks.ts`: Paladin, Haydutlar) | i18n Adım 2 (2026-08-28) | **Şimdilik hayır, kod içinde de değiller.** Zaten `strings.ts` dışında değil, `data/towers.ts`/`data/barracks.ts` içinde — TIER 1 kural 1'in "denge verisi koda gömülmez" ruhuna uygun şekilde **veri** olarak duruyor, sahne kodunda hardcode değil. `guard-rules.mjs`'in yeni i18n kontrolü kasıtlı olarak yalnız `scenes/`+`fx/`'i tarıyor, `data/`'yı değil — bu istisna örtük değil, buraya yazılı. Çevrilecekse (kule dalı adları oyun terminolojisi, sözlük çevirisi değil — `'Barut Fıçısı'` → `'Powder Keg'` gibi ürün kararı gerektiriyor) i18n Adım 3'le birlikte ayrıca ele alınmalı |
