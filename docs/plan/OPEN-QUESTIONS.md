...
| **S82** | ⚠️ **yeni, kapandı** | **Harita 3 HP çarpanı 2,6 → 2,5.** S81'den sonra harita 3 Zor'da (can 12) **14 can** kaybediyordu, yani `difficulty.ts`'in "öğrenme yayı Zor'da geçilebilir" ölçütü düştü. İki ölçüt birden tutmalıydı: Zor'da < 12 **ve** zorluk monoton (h2 < h3 < h4). Tarama: 2,6 → 14 ✗ · 2,55 → 12 ✗ · **2,5 → 11 ✓** · 2,45 → 10 ✓ · 2,4 → 8 ✗ (harita 2 ile eşit). 2,5 iki ölçütü de karşılayan en küçük değişiklik. `GAME-DESIGN` §9 tablosu güncellendi |
...
| **S84** | ⚠️ **yeni, kapandı** | **Kolay hpScale 0,85 → 0,75.** S81'den sonra Kolay'ın kendi ölçütü ("beş harita da bol payla geçiliyor", can kaybı ≤ 10) harita 5'te düştü (13). Tarama: 0,85 → h5 13 ✗ · **0,75 → h5 5 ✓** · 0,65 → h5 2 (neredeyse sızıntısız). 0,75 ölçütü karşılayan en büyük adım |
